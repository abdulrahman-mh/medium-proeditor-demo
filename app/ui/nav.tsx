...
import type { Editor } from 'medium-proeditor';
...
export default function Nav({ editor }: { editor: Editor | null }) {
  ...
  return (
    <div className="flex justify-between items-center [@media(max-width:365px)]:justify-center max-w-[1072px] w-full mx-auto h-[70px] px-4">
      <div>
        <a
          href="https://github.com/abdulrahman-mh/medium-proeditor"
          draggable="false"
          ...
          <Image
            ...
            width={300}
            height={50}
            alt="Medium ProEditor"
            className="pointer-events-none min-w-[250px] w-[250px] md:w-[300px] select-none"
          />
        </a>
      </div>
      ...
      <Modal
        ...
        <ModelHeader
          ...
          onClose={() => setJsonModalOpen(false)}
          title="JSON Content"
          description="The editable document structure, used for saving changes & handling user changes & generating the published HTML."></ModelHeader>
        <pre className="mt-2! overflow-auto h-full w-full flex-1">
          <code
